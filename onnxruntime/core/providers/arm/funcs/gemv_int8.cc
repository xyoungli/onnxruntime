...
inline void WriteGemvOut(const int* in,
                         dtype* out,
                         const float* scale,
                         const float* bias,
                         int size,
...
template <typename dtype>
bool GemvInt8Oth(const int8_t* A,
                 const int8_t* x,
                 dtype* y,
                 bool transA,
                 int M,
                 ...
                 bool is_relu) {
  if (transA) {
    std::cerr << "ERROR: sgemv, transA is not supported now";
    return false;
  }
  dtype* data_out = y;
...
#pragma omp parallel for
  for (int j = 0; j < out_cnt; j++) {
    ...
    WriteGemvOut(ptr_out, out_ptr, scale_ptr, bias_ptr, 8, is_relu);
  }
...
#pragma omp parallel for
  for (int j = out_cnt * 8; j < M; j++) {
    ...
    WriteGemvOut(ptr_out, out_ptr, scale_ptr, bias_ptr, 1, is_relu);
  }
#else  //  __aarch64__
  int out_cnt = M >> 2;
#pragma omp parallel for
  for (int j = 0; j < out_cnt; j++) {
    ...
    WriteGemvOut(ptr_out, out_ptr, scale_ptr, bias_ptr, 4, is_relu);
  }
//! deal with remains
#pragma omp parallel for
...
template <typename dtype>
bool GemvInt8Sdot(const int8_t* A,
                  const int8_t* x,
                  dtype* y,
                  bool transA,
                  int M,
...
template <>
bool GemvInt8<float>(const int8_t* A,
                     const int8_t* x,
                     float* y,
                     bool transA,
                     int M,
...
#if defined(__aarch64__) && defined(USE_ARM_DOTPROD)
  if (ctx->HasDot()) {
    return GemvInt8Sdot<float>(
        A, x, y, transA, M, N, scale, is_bias, bias, is_relu);
  } else {
    return GemvInt8Oth<float>(
        A, x, y, transA, M, N, scale, is_bias, bias, is_relu);
  }
#else
  return GemvInt8Oth<float>(
...
template <>
bool GemvInt8<int8_t>(const int8_t* A,
                      const int8_t* x,
                      int8_t* y,
                      bool transA,
                      int M,
...
#if defined(__aarch64__) && defined(USE_ARM_DOTPROD)
  if (ctx->HasDot()) {
    return GemvInt8Sdot<int8_t>(
        A, x, y, transA, M, N, scale, is_bias, bias, is_relu);
  } else {
    return GemvInt8Oth<int8_t>(
        A, x, y, transA, M, N, scale, is_bias, bias, is_relu);
  }
#else
  return GemvInt8Oth<int8_t>(
