...
Status
LstmOp::Compute(OpKernelContext *context) const {
...
template<typename T>
Status LstmOp::ComputeImpl(OpKernelContext &context) const {
...
void UniDirectionalLstm<T>::Compute(const gsl::span<const T> &inputs_arg,
  ...
  if (batch_parallel_) {
    ...
    ExecuteLambdaInParallel("Processing batch", hidden_gemm_and_activations, batch_size_, fused_hidden_rows, lstm_tp_,
                            logger_);
